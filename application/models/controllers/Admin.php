...
class Admin extends CI_Controller {
	...
	public function New_service() {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($isInsert) {
                $info = ajout_succes_info("Nouveau service");
                $this->session->set_flashdata('info', $info);
...
function get_service() {
	...
	$id_ag = $this->session->userdata('id_ag');
        if ($type==2) {  
$template = array('table_open'=> '<table id="tab" class="well  table table-hover table-striped table-bordered" border=1>');
        $this->table->set_heading(array('#', "DESIGNATION" , 'COUT','EDITION', 'AGENCE', array('data'=>'ACTIONS', 'colspan'=>2 )));
        
        $query_service = $this->userManager->liste_service("SERVICE_AG=$id_ag");
