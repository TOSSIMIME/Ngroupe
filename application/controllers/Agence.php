...
class Agence extends CI_Controller {
    ...
    public function new_agence() {
        ...
        $this->form_validation->set_rules('nom_agence', '"Nom de l\'agence"', 'trim|required|min_length[4]|max_length[52]|encode_php_tags');
         //$this->form_validation->set_rules( 'contact_agence',"Contact de l\'agence", 'trim|required|min_length[2]|max_length[8]|is_numeric|encode_php_tags');
        $this->form_validation->set_rules( 'email_agence',"Email agence", 'trim|min_length[5]|max_length[50]|encode_php_tags');
         $this->form_validation->set_rules( 'adresse_agence',"Adresse agence(BP)", 'trim|min_length[2]|max_length[50]|encode_php_tags');
         $this->form_validation->set_rules( 'siege_agence',"SIEGE AGENCE", 'trim|min_length[2]|max_length[50]|encode_php_tags');
         $this->form_validation->set_rules( 'etat',"Statut agence", 'trim|min_length[1]|max_length[50]|encode_php_tags');
...
$isInsert = $this->agenceManager->ajouter_agence($nom_agence, $contact_agence, $email_agence,$adresse_agence,$siege_agence,$photoNom, $etat );
    ...
    public function update($id = '') {
        ...
        $this->form_validation->set_rules('nom_agence', '"Nom de l\'agence"', 'trim|required|min_length[4]|max_length[52]|encode_php_tags');
        $this->form_validation->set_rules('contact_agence', '"Contact agence"', 'trim|required|min_length[4]|max_length[52]|encode_php_tags');
            if ($this->form_validation->run()) {
                $isInsert = FALSE;
                //recuperation valeur
          $nom_agence = $this->input->post('nom_agence');
 ...
 $isInsert = $this->agenceManager->update_agence($id ,$nom_agence, $contact_agence, $email_agence,$adresse_agence,$siege_agence,$photoNom, $etat );
                if ($isInsert) {
...
$template = array('table_open'=> '<table id="tab" class="  table table-hover table-striped table-bordered" border=1>' );
        $this->table->set_heading(array('Agence_de_voyages ','Contacts', "E-mail" ,'Siege',  array('data'=>'Actions','colspan'=>2)));
...
function get_liste() {
    $type = $this->session->userdata('type');
        if ($type==1) {  
$template = array('table_open'=> '<table id="tab" class="  table table-hover table-striped table-bordered" border=1>' );
        $this->table->set_heading(array('Agence_de_voyages ','Contacts', "E-mail" ,'Siege',  'Quota pelerin','Quota vols','Aperçu'));
...
$template = array('table_open'=> '<table id="tab" class="  table table-hover table-striped table-bordered" border=1>' );
        ...
        if (is_array($query_agence) && count($query_agence)) {
            ...
            foreach ($query_agence as $loop) {
                ...
                $loop->SIEGE_AG,'<input type="submit"  value="'.$loop->QUOTA_AG.'" class=" btn btn-info" />'));
            }
        }
